...
- [2026-05-06] Tool modules that consume monkey-patchable hooks from `arxiv_deep.tools.fetch` import the *module* (`from arxiv_deep.tools import fetch as _fetch`) and reference attributes at call time (`_fetch._download_pdf(...)`). Reason: a `from arxiv_deep.tools.fetch import _download_pdf` form binds the function reference at import time, so tests that do `monkeypatch.setattr(fetch_mod, "_download_pdf", fake)` silently fail to take effect — the consumer's local name still points at the real function. This bit `extract_figures` and shipped a false-hermetic test that hit live arxiv on every run; CI caught it the day arxiv rate-limited macos-14/py3.12. The fix is a one-line import change plus a `block_arxiv_api` conftest fixture that opt-in tests use as a regression sentinel against this class of bug.
- [2026-05-06] Phase 4 demo pivots from `Qwen3-VL-2B` on Oxford Pets to `HuggingFaceTB/SmolLM2-135M-Instruct` on `tatsu-lab/alpaca`. Reason: `transformers` 5.x has a regression in the auto-loaded image-processor chain that affects every multimodal preset tested (SmolVLM, Qwen2-VL, Qwen3-VL); pinning to a v4.x is brittle. The pivot keeps the demo's narrative ("agent reads paper, runs three LoRA variants, picks winner, publishes") and gives a 25× iteration-speed-up (cold-start training in ~15s vs. multi-minute VLM loads). Documented in `demo/README.md` under "Why this model and dataset"; the original CLAUDE.md decision (Qwen3-VL-2B, Oxford Pets, LoRA ranks 4/8/16) is superseded.
- [2026-05-06] Demo training scripts publish to the authenticated user's HF namespace by default; `DEMO_PUSH_TO_ORG=1` opts in to the `ml-agent-explorers` org. Reason: demo iterations should not leak half-baked artefacts into a public org; the personal namespace is the safe default. The `_push_adapter` helper refuses to overwrite an existing repo (raises with a clear message) so even within a namespace re-runs are explicit.
- [2026-05-06] Demo scripts (`demo/scripts/`) are excluded from `mypy --strict`. Reason: standalone entry points consumed by an LLM agent over a CLI / single-line-JSON contract have a different typing bar from library code that humans import; the scripts pull in `torch`/`transformers`/`peft` whose stubs are partial and not worth hand-rolling. Tests for the orchestration logic are torch-free and pass under strict; integration tests gated on `PYTEST_INTEGRATION=1` exercise the real training path.
- [2026-05-06] Demo orchestrator passes the prompt as the positional argument to `ml-intern` (`uv run ml-intern "$(cat prompt.txt)"`), not via a `--prompt-file` flag. Reason: PROMPTS.md's literal `--prompt-file` instruction does not match upstream's argparse (which only takes a positional `prompt`). Confirmed against `agent/main.py:1422` during Phase 3.1.
- [2026-05-06] HF Jobs path uses `huggingface_hub.run_uv_job` to fetch `train_local.py` from main and execute it inside a uv-managed runner, then polls `inspect_job` and parses the contract-line out of `fetch_job_logs`. Reason: one source of truth for training (the local script). HF Jobs is a transport, not a separate codepath. Trade-off: the runner pulls the script from the public main branch, so changes to the script must land before any `DEMO_MODE=hf-jobs` run picks them up.
